...
TX_MUTEX gFlashGlobalLock;
TX_EVENT_FLAGS_GROUP gFlashEventFlags;
...
void flashISR()
{
	...
	//no need to handle those commands since they block, signal to the waiting thread that he may continue
	if (gCurrentCommand == BLOCKING_READ_DATA || gCurrentCommand == BLOCKING_PAGE_PROGRAM || gCurrentCommand == IDLE)
	{
		tx_event_flags_set(&gFlashEventFlags,1,TX_OR);
		return;
	}
...
bool flash_is_ready(void)
{
	...
	do
	{
		if (tx_mutex_get(&gFlashGlobalLock,TX_WAIT_FOREVER) != TX_SUCCESS)
		{
			break;
		}
		
		//hardware idle and we are not in a middle of big operation
		isReady = (gCurrentCommand==IDLE && flashIsIdle());
		
		tx_mutex_put(&gFlashGlobalLock);
